# Agent Learning Kit Deep Dive

This document preserves the detailed CLI and SDK cookbook material that used to live in the public README. Keep root README focused on public onboarding and move long-form release-gate or cookbook detail here.

## Deep Dive
The rest of this README documents the detailed CLI and SDK surfaces. It is
intentionally long because each example is meant to be directly runnable.
...
If you have a local object but are not sure which method/input shape to probe,
use `simulate.discover_framework_adapter()` first. It inspects callable
attributes without importing or executing the framework, rejects HTTP targets by
default, and returns both ranked discovery contracts and an `adapter_candidates`
list that can be passed directly to `optimize.optimize_framework_adapter_probe`:
...
assert discovery["adapter_candidates"][0] == {
    "method": "execute_task",
    "input_mode": "dict",
    "trace_runtime": True,
    "target": "framework_shims.py:build_custom_refund_orchestrator",
...
LiveKit session adapters are release-gated through the same BYO path.
`examples/sdk_framework_adapter_livekit_run_session_promotion.py` requires local
discovery to include a session-shaped `run_session(dict)` candidate, reject the
weak `respond(text)` fallback, preserve `simulation.modality=voice`, emit
`framework_trace`, `livekit_session_event`, and `livekit_transcript` events, and
close the normal promoted-run metric floors without calling LiveKit Cloud.
...
Local WebSocket framework transports prove the realtime sibling path.
`simulate.build_framework_websocket_transport_run_manifest(...)` builds a
loopback-only `agent.type=websocket` manifest using the same Agent Learning
protocol, bearer auth from
`AGENT_LEARNING_SDK_FRAMEWORK_WEBSOCKET_TRANSPORT_KEY`, accepted handshake and
JSON-frame evidence, framework runtime state, framework trace events/artifacts,
and `framework_websocket_status` tool evidence. See
`examples/sdk_framework_adapter_websocket_transport.py`. `agent-learn
release-check` runs this cookbook as `framework_websocket_transport_readiness`
and adds it to the 10x robustness bar as
...
target path as separate LangGraph, CrewAI, LlamaIndex, LangChain, Pipecat, and
LiveKit profiles and gates them as `workflow_target_profile_matrix_readiness`,
so Agent Learning remains the primary framework while external shapes stay
compatibility inputs. The result renders a `workflow_target_profile_matrix`
report/action card with exportable summary, profile evidence, and replay-lock
artifacts.
