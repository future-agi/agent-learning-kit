...
## Writing setup, ready and the solution

Getting these wrong is the commonest way a scenario is rejected: setup composes against the frozen
base and not against the scenario you wrote before it, neither way of changing state names what the
world is kept in, and the solution is what proves the scenario before it is kept. Read
`references/_authoring-code.md` at the point of writing them.
