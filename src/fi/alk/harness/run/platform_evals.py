...
def instructions_for(claim: str, agent: str, rules: list[str] | None = None) -> str:
    ...
    return (
        ...
        "what the agent ended up doing, not what it tried on the way. Apply the claim's own "
        "criteria exactly, including what they say when its situation does not arise, and add "
        "nothing they do not state; only the agent is judged. Declining something holds only if "
        "the agent "
