...
def _judge_prompt(contract: AgentContract) -> str:
    return (
        ...
        "settle, with its own criteria: when it applies, what passes, what fails and what the "
        "verdict is when its situation does not arise. Judge each independently, only from the "
        "evidence in front of you, and exactly by its criteria: add nothing they do not state. "
        "Only the agent is judged; what the person set out to do is context.\n\n"
