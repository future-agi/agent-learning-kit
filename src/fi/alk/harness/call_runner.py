...
# --- credential aliases / config keys (the exact list Khushal configures the UI upload from) ---
...
# CONTRACT NOTE 2: unpinned by any contract, no producer exists yet. Isolated as one
...
def _dispatch_agent_name(runtime: EnvironmentRuntime) -> str | None:
    """CONTRACT NOTE 3: the ONLY place this repo reads the dispatch-identity
    metadata key, so a change to the key name/convention (still being confirmed with Azain) is a
    one-line adapt. Verified finding: `EnvironmentRuntime.metadata` defaults to `{}` and nothing
    in `process_runtime.py` populates it as of this worktree's HEAD -- every real "livekit" job
...
def _simulator_definition() -> Any:
    """CONTRACT NOTE 5: mirrors `sdk_voice.py::_simulator()`'s own shipped defaults
...
def _collect_http_tool_calls(runtime: EnvironmentRuntime) -> tuple[Call, ...]:
    """CONTRACT NOTE 1: no guest-side capture surface exists anywhere in this repo for the
...
def _clear_tool_trace_calls(dsn: str) -> None:
    ...
    producer yet -- CONTRACT NOTE 2) or any connection error is swallowed, never raised. Clearing
...
def _collect_tool_trace_calls(runtime: EnvironmentRuntime) -> tuple[Call, ...]:
    """CONTRACT NOTE 2: `_alk_tool_trace`'s name and column shape are an isolated local
...
class CallRunnerImpl:
    ...
    async def run(self, scenario: HostedScenario, runtime: EnvironmentRuntime) -> CallOutcome:
        if self._missing_config is not None:
            ...
            # job-level voice config gap; CONTRACT NOTE 4).
        ...
        if agent_name is None:
            raise CallAborted(
                ...
                f"not set for world {runtime.world_index} -- CONTRACT NOTE 3"
            )
