...
@pytest.mark.parametrize("stop_after", [None, 31])
def test_generated_rows_are_durably_metered_until_completion_or_cancel(tmp_path, stop_after):
    import asyncio
    from fi.alk.harness.scenario_tools import accept_scenario
    from fi.alk.harness.usage import UsageJournal, configure_reporter

    root, _contract, catalogue = _built_environment(tmp_path)
    journal = UsageJournal(tmp_path / "usage.json", attempt_id="generation-test")
    ...
    try:
        try:
            for index in range(50):
                accept_scenario(_delta(name=f"case-{index}"), world_root=root, catalogue=catalogue, kept=kept)
        except asyncio.CancelledError:
            assert stop_after is not None
    finally:
        configure_reporter(None)

    expected = stop_after or 50
    restored = UsageJournal(journal.path, attempt_id="generation-test")
    assert len(restored.records) == expected
    assert sum(record.amount for record in restored.records) == expected
    assert {record.scenario_key for record in restored.records} == {row.name for row in kept}
    assert len(kept) == expected
    assert all((root / "scenarios" / row.name / "scenario.json").exists() for row in kept)


def test_scenario_usage_is_recorded_on_new_row_not_replacement(tmp_path):
    from fi.alk.harness.scenario_tools import accept_scenario
    from fi.alk.harness.usage import UsageJournal, configure_reporter

    root, _contract, catalogue = _built_environment(tmp_path)
    journal = UsageJournal(tmp_path / "usage.json", attempt_id="replacement-test")
    ...
    configure_reporter(Reporter())
    try:
        accept_scenario(_delta(), world_root=root, catalogue=catalogue, kept=kept)
        accept_scenario(_delta(), world_root=root, catalogue=catalogue, kept=kept)
    finally:
        configure_reporter(None)
    assert len(journal.records) == 1
    assert sum(record.amount for record in journal.records) == 1
    assert [row.name for row in kept] == ["adds-a-big-mac"]


def test_scenario_budget_denial_keeps_thirtyeight_produced_rows(tmp_path):
    from fi.alk.harness.scenario_tools import accept_scenario
    from fi.alk.harness.usage import UsageDenied, UsageJournal, configure_reporter

    root, _contract, catalogue = _built_environment(tmp_path)
    journal = UsageJournal(tmp_path / "usage.json", attempt_id="limit-test")

    class Reporter:
        def check(self, action, **kwargs):
            if len(journal.records) == 38:
                raise UsageDenied({"error_code": "FREE_TIER_LIMIT", "dimension": "ai_credits", "reason": "scenario budget exhausted"})
    ...
    configure_reporter(Reporter())
    try:
        with pytest.raises(UsageDenied) as refused:
            for index in range(50):
                accept_scenario(_delta(name=f"case-{index}"), world_root=root, catalogue=catalogue, kept=kept)
        assert refused.value.detail["dimension"] == "ai_credits"
    finally:
        configure_reporter(None)
    assert len(kept) == len(journal.records) == 38
    assert sum(record.amount for record in journal.records) == 38
    assert not (root / "scenarios" / "case-38").exists()
