...
def test_a_consumable_process_wired_via_sh_c_and_env_dollar_key_passes(
    tmp_path: Path,
) -> None:
    ...
    assert (
        preflight_bundle(
            tmp_path, manifest, parallelism=4, secret_refs=TARGET_PROVIDER_REFS
        )
        is None
    )


def test_a_consumable_token_only_in_an_unreferenced_env_var_is_unwired(
    tmp_path: Path,
) -> None:
...
def test_a_consumable_token_placed_directly_in_the_argv_is_unwired(
    tmp_path: Path,
) -> None:
...
def test_two_processes_declaring_the_same_fixed_port_is_a_duplicate(
    tmp_path: Path,
) -> None:
...
def test_agent_name_missing_job_id_at_w_gt_1_is_rejected(tmp_path: Path) -> None:
    def mutate(body: dict[str, Any]) -> dict[str, Any]:
        body["processes"][1]["environment"]["LIVEKIT_AGENT_NAME"] = (
            "agent-w{{WORLD_INDEX}}"
        )
        return body
...
def test_agent_name_with_both_placeholders_passes_at_high_w(tmp_path: Path) -> None:
    ...
    assert (
        preflight_bundle(
            tmp_path, manifest, parallelism=8, secret_refs=TARGET_PROVIDER_REFS
        )
        is None
    )
