...
The harness installs the editable parent `agent-learning-kit` package and the UI's `fastapi` and
`uvicorn` dependencies from its own manifest.
...
**Restart the server after changing anything under `src/agent_harness/`.** A long-lived process
