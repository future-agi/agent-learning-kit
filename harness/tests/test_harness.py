...
from agent_harness.session import ARTIFACT, DONE, TEXT, TOOL, Event
from agent_harness.tools import accept_contract, qualified
from agent_harness.understand import load, opening
...
def _cart_world():
    from agent_harness.world import GeneratedWorld
...
def test_a_sound_world_passes_every_probe():
    from agent_harness.world import probe
...
def _saved_world(tmp_path):
    from agent_harness.world.snapshot import save
...
def test_declared_types_become_something_a_tool_schema_can_carry():
    from agent_harness.run.targets import _python_type
...
def test_the_agent_under_test_is_told_its_own_rules():
    from agent_harness.run.targets import agent_prompt
...
def test_a_conversation_resumes_at_whichever_stage_the_artifacts_reached(tmp_path):
    from agent_harness.chat import BUILD, SCENARIOS, UNDERSTAND, open_conversation
...
def test_where_a_conversation_is_agrees_with_what_was_built(tmp_path):
    from agent_harness.chat import SCENARIOS, open_conversation
...
def test_a_conversation_with_no_agent_starts_by_asking_which_one():
    from agent_harness.chat import RECEPTION, open_conversation
...
def test_pointing_at_an_agent_settles_where_its_artifacts_go(tmp_path):
    ...
    from agent_harness.chat import UNDERSTAND, open_conversation
    from agent_harness.sources import RepoSource
...
def test_pointing_at_somewhere_that_does_not_exist_is_refused(tmp_path):
    from agent_harness.reception import point_at
...
def test_pointing_at_a_github_url_clones_it_into_the_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("agent_harness.sources.subprocess.run", clone)
    found = {}
...
def test_how_many_scenarios_is_something_you_say():
    from agent_harness.scenario_tools import TOOL_NAMES
...
def test_the_agent_can_be_taught_a_value_it_did_not_accept(tmp_path):
    ...
    from agent_harness.amend import widen
...
def test_a_stage_may_use_nothing_it_was_not_given():
    ...
    from agent_harness.config import permission_gate
...
def test_every_stage_publishes_exactly_the_tools_it_claims(tmp_path):
    ...
    from agent_harness import scenario_tools as scenarios
    from agent_harness.run import tools as runs
    from agent_harness.world import tools as world
...
def test_a_failed_call_is_not_reported_as_success():
    ...
    from agent_harness.session import _why_it_failed
...
def test_the_credentials_in_play_are_said_out_loud(monkeypatch):
    from agent_harness.config import credentials_hint
...
def test_a_run_notices_when_it_was_billed_to_a_model_nobody_asked_for():
    ...
    from agent_harness.session import Stage
...
def test_an_agent_already_built_can_be_reopened_without_its_repository(tmp_path):
    ...
    from agent_harness.chat import SCENARIOS, Conversation
...
def test_a_rule_the_source_never_stated_can_be_added_and_is_recorded(tmp_path):
    ...
    from agent_harness.amend import add_rule
...
def test_a_collection_is_read_and_written_in_the_same_place():
    ...
    from agent_harness.world.runtime import GeneratedWorld
    from agent_harness.world.stores import open_store
...
def test_the_modality_can_be_corrected_when_the_source_reads_the_other_way(tmp_path):
    ...
    from agent_harness.amend import set_modality
...
def test_a_rule_the_agent_does_not_have_can_be_taken_away(tmp_path):
    ...
    from agent_harness.amend import add_rule, drop_rule
...
def test_a_misread_tool_can_be_corrected(tmp_path):
    ...
    from agent_harness.amend import fix_tool
...
def test_correcting_a_contract_without_saying_why_is_refused(tmp_path):
    from agent_harness.amend import drop_rule, fix_tool
...
def test_a_row_put_in_wrong_can_be_taken_out_again(tmp_path):
    ...
    from agent_harness.world import tools as world_tools
...
def test_a_sub_goal_that_settles_nothing_is_rejected():
    ...
    from agent_harness.catalogue import SubGoal, validate_sub_goal
...
def test_a_simulator_prompt_without_a_slot_runs_the_same_conversation_every_time():
    from agent_harness.simulator import fill, validate_simulator_prompt, variables_in
...
def test_a_conversational_simulator_prompt_requires_a_persona_slot():
    from agent_harness.simulator import validate_simulator_prompt
...
def test_a_persona_is_a_structured_simulator_prompt_slot():
    from agent_harness.scenario import Persona, Scenario
    from agent_harness.simulator import fill
...
def test_an_empty_persona_is_rejected():
    from agent_harness.catalogue import Catalogue, SubGoal
    from agent_harness.scenario import Persona, Scenario, validate_scenario
...
def test_a_check_that_raises_is_broken_not_failed():
    ...
    from agent_harness.checks import run_check
...
def _built_environment(tmp_path):
    ...
    from agent_harness.catalogue import Catalogue, SubGoal, save_catalogue
    from agent_harness.world.snapshot import save
...
def test_a_scenario_is_proved_before_it_is_kept(tmp_path):
    from agent_harness.scenario_tools import accept_scenario
...
def test_a_scenario_whose_checks_pass_with_nothing_done_is_refused(tmp_path):
    ...
    from agent_harness.catalogue import SubGoal, save_catalogue
    from agent_harness.scenario_tools import accept_scenario
...
def test_a_check_that_cannot_fail_without_calls_is_named_even_though_it_is_kept(tmp_path):
    ...
    from agent_harness.catalogue import SubGoal, save_catalogue
    from agent_harness.prove import prove
    from agent_harness.scenario import Scenario
    from agent_harness.scenario_tools import accept_scenario
...
def test_a_store_that_is_not_sqlite_needs_nothing_above_it_to_change(tmp_path):
    ...
    from agent_harness.world.runtime import GeneratedWorld
    from agent_harness.world.stores import open_store, register_store
    ...
    class Ledger:
        ...
        def freeze(self):
            from agent_harness.world.stores import Snapshot
...
def test_a_world_check_that_cannot_fail_is_named(tmp_path):
    ...
    from agent_harness.checks import run_world_check
    from agent_harness.world.mutate import blind, unnoticed
    from agent_harness.world.runtime import GeneratedWorld
    from agent_harness.world.snapshot import restore, save
...
def test_the_world_keeps_the_checks_that_prove_it(tmp_path):
    ...
    from agent_harness.world.runtime import GeneratedWorld
    from agent_harness.world.snapshot import read_manifest, restore, save
...
def test_every_contract_field_is_advertised_to_the_model(tmp_path):
    ...
    from agent_harness.contract import AgentContract

    from pydantic import BaseModel

    advertised = (
        Path(__file__).resolve().parents[1] / "src" / "agent_harness" / "tools.py"
...
def test_the_adoption_fields_survive_the_write_path(tmp_path):
    ...
    from agent_harness.tools import accept_contract
    from agent_harness.understand import load
...
def test_a_suite_where_no_sub_goal_is_shared_does_not_roll_up(tmp_path):
    ...
    from agent_harness.catalogue import Catalogue, SubGoal
    from agent_harness.scenario import Scenario
    from agent_harness.scenario_tools import not_ready
...
def test_the_simulator_prompt_slots_a_scenario_leaves_unfilled_are_caught(tmp_path):
    from agent_harness.scenario import Scenario, validate_scenario
...
def test_a_hosted_agents_tool_call_is_answered_by_the_world():
    ...
    from agent_harness.run.voice import WorldWebhook
...
def test_repointing_changes_only_where_the_agents_tools_are_answered():
    ...
    from agent_harness.run.voice import pointed_at
...
def test_a_scenario_fills_the_simulator_prompt_before_a_call_is_placed(tmp_path):
    from agent_harness.simulator import save_simulator_prompt
    from agent_harness.run.live import prepare
    from agent_harness.scenario import Scenario
...
def test_a_live_run_is_refused_before_it_costs_anything(monkeypatch):
    ...
    from agent_harness.run.tools import missing_prerequisites
...
def test_running_is_a_stage_of_the_conversation():
    ...
    from agent_harness import chat
...
def test_a_run_result_survives_being_written_and_read(tmp_path):
    from agent_harness.checks import Outcome
    from agent_harness.run.live import LiveRun
    from agent_harness.run.tools import as_record, load_results, save_results
...
def test_a_tool_a_stage_was_not_given_is_denied_by_the_hook():
    ...
    from agent_harness.config import gate_hooks
...
def test_every_stage_gates_with_the_hook_not_only_the_callback():
    ...
    from agent_harness import build, reception, scenarios
    from agent_harness.run import grade, stage, targets
...
def test_writing_new_results_keeps_the_ones_not_rerun(tmp_path):
    ...
    from agent_harness.run.tools import load_results, save_results
...
def test_submit_contract_schema_teaches_and_leaves_gating_to_the_gate(tmp_path):
    ...
    from agent_harness.contract import MODALITIES
    from agent_harness.tools import contract_tools
...
def test_handoff_is_refused_until_the_stage_has_its_artifact(tmp_path):
    ...
    from agent_harness.chat import Conversation
...
def test_a_contract_sent_inside_a_wrapper_is_unwrapped(tmp_path):
    ...
    from agent_harness.tools import accept_contract, unwrapped
...
def test_a_skill_only_names_tools_its_stage_actually_has():
    ...
    from agent_harness import scenario_tools
    from agent_harness.config import SKILLS_ROOT
    from agent_harness.run import tools as run_tools
    from agent_harness.tools import CONTRACT_SERVER  # noqa: F401
    from agent_harness.world import tools as world_tools
    ...
    from agent_harness.contract import AgentContract, ToolSpec
    from agent_harness.catalogue import SubGoal
    from agent_harness.scenario import Persona, Scenario
...
def test_only_a_tool_that_says_it_saved_reports_an_artifact():
    ...
    from agent_harness.session import _saved_path
...
def test_the_setups_own_calls_are_not_credited_to_the_agent(tmp_path):
    ...
    from agent_harness.prove import prepared
...
def test_a_kept_scenario_becomes_a_folder_of_files(tmp_path):
    ...
    from agent_harness.folder import folder_for, read_folder
    from agent_harness.scenario_tools import write_scenarios
...
def test_a_check_file_runs_on_its_own_and_agrees_with_the_harness(tmp_path):
    ...
    from agent_harness.folder import folder_for, write_folder
    from agent_harness.prove import prepared
...
def test_a_session_is_a_folder_that_knows_what_it_holds(tmp_path):
    ...
    from agent_harness import sessions
...
def test_the_turn_that_finds_the_agent_also_opens_the_next_stage(tmp_path, monkeypatch):
    ...
    from agent_harness import chat as chat_module
    from agent_harness.chat import Conversation
    from agent_harness.sources import RepoSource
...
def test_the_build_skill_documents_every_method_a_handler_can_call():
    ...
    from agent_harness.world.runtime import Db
...
def test_a_reset_that_forgets_its_counters_is_caught_without_naming_one(tmp_path):
    ...
    """
    from agent_harness.world.stores import resolve
    from agent_harness.world.stores.prove import prove_store
...
def test_an_engine_nobody_taught_the_harness_is_refused_by_name():
    ...
    from agent_harness.world.stores import StoreError, resolve, supported
...
def test_a_written_store_has_to_say_how_a_scenario_changes_it():
    ...
    """
    from agent_harness.world.stores import StoreError
    from agent_harness.world.stores.written import register_written
...
def test_emptying_a_store_is_something_restore_can_reproduce():
    ...
    from agent_harness.world.stores import Snapshot, resolve
...
def test_a_missing_store_names_the_root_and_what_is_under_it(tmp_path):
    ...
    from agent_harness.world.tools import _stores_here
...
def test_a_tool_that_cannot_be_reached_has_a_way_out_that_is_recorded(tmp_path):
    ...
    from agent_harness.amend import unreachable
    from agent_harness.contract import AgentContract
...
def test_emptying_a_world_survives_foreign_keys(tmp_path):
    ...
    """
    from agent_harness.world.mutate import _empty, left
    from agent_harness.world.runtime import GeneratedWorld
...
def test_a_mutation_that_does_not_land_accuses_nobody():
    ...
    from agent_harness.world.mutate import EMPTIED, SILENCED, UNDAMAGED, blind, unnoticed
    from agent_harness.world.runtime import GeneratedWorld
...
def test_an_opening_line_in_the_agents_voice_falls_back_to_the_instruction():
    ...
    """
    from agent_harness.run.conversation import OPENING, _answered_as_the_agent
...
def test_a_refusal_scenario_is_not_vacuous_because_its_evidence_is_what_was_said():
    ...
    from agent_harness.checks import Outcome
    from agent_harness.catalogue import Catalogue, SubGoal
    from agent_harness.prove import Proof
...
def test_a_setup_or_ready_that_says_nothing_is_not_a_complaint():
    ...
    from agent_harness.folder import _run
...
def test_an_optional_field_may_be_null():
    ...
    from agent_harness.tools import schema
...
def test_dropping_a_scenario_removes_it_from_disk(tmp_path):
    ...
    from agent_harness.catalogue import Catalogue, SubGoal
    from agent_harness.scenario import Scenario
    from agent_harness.scenario_tools import load_scenarios, write_scenarios
...
def test_a_target_refuses_a_model_it_cannot_drive():
    ...
    """
    from agent_harness.run.targets import _drivable
...
def test_a_run_is_a_folder_that_can_be_read_back(tmp_path):
    ...
    from agent_harness.run.grade import Result
    from agent_harness.run.simulation import _write_case, every_run, read_run, run_root
...
def test_the_closing_line_is_kept():
    ...
    """
    from agent_harness.run.conversation import DONE, STUCK, customer_prompt
...
def test_a_judged_sub_goal_becomes_a_named_platform_eval():
    ...
    from agent_harness.run import platform_evals
...
def test_a_verdict_is_read_however_it_arrives():
    ...
    from agent_harness.run.platform_evals import _passed
...
def test_voice_suite_evals_use_the_documented_platform_inputs(monkeypatch):
    from agent_harness.catalogue import default_suite_evals
    from agent_harness.contract import AgentContract
    from agent_harness.run.conversation import Exchange, Transcript
    from agent_harness.run.grade import judge_suite_evals
    from agent_harness.run import platform_evals
    from agent_harness.scenario import Scenario
...
def test_suite_evals_do_not_run_for_non_voice_agents(monkeypatch):
    from agent_harness.catalogue import default_suite_evals
    from agent_harness.contract import AgentContract
    from agent_harness.run.conversation import Transcript
    from agent_harness.run.grade import judge_suite_evals
    from agent_harness.run import platform_evals
    from agent_harness.scenario import Scenario
