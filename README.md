...
90 local metrics, guardrail scanners, streaming assessment, and cloud scoring — one `evaluate()` call.
...
- **Unified `evaluate()` API** — one function, 90 local metrics, local or cloud
...
- [Local Metrics](#local-metrics--90-metrics-zero-network-calls)
...
## Local Metrics — 90 metrics, zero network calls
...
- [x] Unified `evaluate()` API with 90 local metrics
