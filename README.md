...
78 local metrics, guardrail scanners, streaming assessment, and cloud scoring — one `evaluate()` call.
...
- **Unified `evaluate()` API** — one function, 78 local metrics, local or cloud
...
- **Agent World/Replays** — score `world_attack_replay`, `world_orchestration_replay`, optimizer society traces, framework runtime contracts, and observability replay-pack artifacts with nested orchestration, world-contract, optimizer-role, adversarial, metric, trace-signal, and raw regression evidence
...
- [Local Metrics](#local-metrics--78-metrics-zero-network-calls)
...
## Local Metrics — 78 metrics, zero network calls
...
- [x] Unified `evaluate()` API with 78 local metrics
