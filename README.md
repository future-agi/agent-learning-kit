...
82 local metrics, guardrail scanners, streaming assessment, and cloud scoring — one `evaluate()` call.
...
- **Unified `evaluate()` API** — one function, 82 local metrics, local or cloud
...
- **Agent World/Replays** — score `world_attack_replay`, `world_orchestration_replay`, optimizer society/governance traces, framework runtime contracts, framework lifecycle/session traces, framework capability matrices, and observability replay-pack artifacts with nested orchestration, world-contract, optimizer-role, lifecycle, capability, adversarial, metric, trace-signal, and raw regression evidence
...
- [Local Metrics](#local-metrics--82-metrics-zero-network-calls)
...
## Local Metrics — 82 metrics, zero network calls
...
- [x] Unified `evaluate()` API with 82 local metrics
