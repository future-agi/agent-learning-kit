...
80 local metrics, guardrail scanners, streaming assessment, and cloud scoring — one `evaluate()` call.
...
- **Unified `evaluate()` API** — one function, 80 local metrics, local or cloud
...
- [Local Metrics](#local-metrics--80-metrics-zero-network-calls)
...
## Local Metrics — 80 metrics, zero network calls
...
- [x] Unified `evaluate()` API with 80 local metrics
